...
function formatUseRong(val, row, index) {
    var type = parseInt(val);
    if (type == 0)
        return '<button type="button" onclick="doUseRong(' + "'" + row.id + "'" + ')">使用融云</button>';
    else
        return '<span style="color:blue;">已使用融云</span>';
}

function doUseRong(id) {
